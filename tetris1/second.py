...
def playing():
    ...
    field = [[0 for i in range(W)] for j in range(H)]#двухмерный массив, заполненный нулям, карты
    # игрогого поля(минута 5.03), на котором отмечаем положение упавших фигур

    count_animation, speed_animation, limit_animation = 0, 60, 2000
    figure = deepcopy(choice(figures))#начальную фигуру делаем случайной
    ...
    def check_borders():
        if figure[i].x < 0 or figure[i].x > W - 1:
            ...
            return False # новыые условии для проверки границ, возвращаем ложное значение, когда фигура
            # достигла дна, и если на игровом поле уже лежит другая фигура на этих координатах
        return True
    ...
    while game_over == 0:
        ...
        def show_and_record_score(score1, record1):
        ...
        if count_animation > limit_animation:
            ...
            for i in range(4):
                ...
                if not check_borders():
                    for i in range(4):
                        field[figure_old[i].y][figure_old[i].x] = seven_color[figure_index] # выполняется
                        # это условие то мы заносим в карту игрового поля цвет упавшие фигуры
                    figure = deepcopy(choice(figures)) # выбираем новую случайную фигуру при приземление
                    # текущей и раскрашиваем ее
                    figure_index = figures.index(figure)
        ...
        # отрисовка поля с же упавшими фигурами
        for y, raw in enumerate(field): # идем в циклах по элем. и там где вместо 0 хран. цвет,
            # используя коорд. рисуем квадрат того же цвета
            for x, col in enumerate(raw):
                if col:
                    figure_rect.x, figure_rect.y = x * TILE, y * TILE
                    pygame.draw.rect(game_sc, col, figure_rect)

        show_and_record_score(score, check_record()) # выводим информацию на экран
